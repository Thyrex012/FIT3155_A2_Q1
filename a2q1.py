...
class Node:
    def __init__(self, start, end, suffix_link = None, isLeaf = False):
...
class Ukkonen_algorithm:
    def __init__(self):
        self.root = Node(-1, -1, isLeaf = False)
    ...
    def rule_two_regular(active_node, ):
        return
    ...
    def rule_two_alternate(self, active_node: Node, start: int, txt_with_dollar, new_active_node, new_remainder):
        ...
        # If the previous phase's pending node hasn't been resolved yet then the active node
